...
## 3. 하네스 엔지니어링 방어 게이트 (Harness Gates)
- **부탁이 아닌 차단**: 문서는 확률을 낮추고, 게이트는 확정으로 막는다.
- **1층 실행 게이트**: 대표님 명령 화이트리스트 키워드가 부재할 시 도구 호출 자체를 차단.
- **2층 커밋 게이트**: `npm run build` 0에러 미통과 시 Git 커밋 영구 거부.
- **3층 배포 게이트**: 다중 뷰포트 헤드리스 실측 캡처로 0 regression 증명 전 완료 선언 금지.
- **스키마 하향 안전망**: DB 컬럼/필드 추가 시 기존 레거시 데이터와의 하위 호환성을 위해 Optional Chaining(`?.`) 및 기본 Fallback 값을 코드에 필수 강제.
