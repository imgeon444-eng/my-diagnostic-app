...
export default function StorybookModal({ onClose }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'Escape') onClose(); // ESC 누르면 닫히기
    };
      ...
      case 'cover':
        return (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            <div className="p-4 md:p-6 bg-emerald-500/10 rounded-full mb-4">
              <Target className="w-12 h-12 md:w-16 md:h-16 text-emerald-400" />
            </div>
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-white tracking-tight mb-4 break-keep">{slide.title}</h1>
            <h2 className="text-xl md:text-2xl text-emerald-400 font-bold mb-6 break-keep">{slide.subtitle}</h2>
            <p className="text-base md:text-xl text-slate-400 max-w-2xl leading-relaxed break-keep">{slide.description}</p>
          </div>
        );
      case 'comparison':
        return (
          <div className="flex flex-col h-full w-full justify-center px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
              <div className="bg-slate-800/50 p-6 md:p-8 rounded-2xl border border-slate-700">
                <h3 className="text-xl md:text-2xl font-bold text-sky-400 mb-4 md:mb-6">{slide.left.title}</h3>
                <ul className="space-y-3 md:space-y-4">
                  {slide.left.items.map((item, idx) => (
                    <li key={idx} className="flex items-start text-slate-300 text-sm md:text-lg break-keep">
                      <span className="mr-3 text-sky-400 mt-1">•</span>{item}
                    </li>
                  ))}
                </ul>
              </div>
              <div className="bg-slate-800/80 p-6 md:p-8 rounded-2xl border border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.1)]">
                <h3 className="text-xl md:text-2xl font-bold text-emerald-400 mb-4 md:mb-6">{slide.right.title}</h3>
                <ul className="space-y-3 md:space-y-4">
                  {slide.right.items.map((item, idx) => (
                    <li key={idx} className="flex items-start text-slate-200 text-sm md:text-lg break-keep">
                      <CheckCircle className="w-5 h-5 md:w-6 md:h-6 mr-3 text-emerald-500 shrink-0 mt-0.5" />{item}
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="flex flex-col h-full w-full justify-center px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {slide.cards.map((card, idx) => (
                <div key={idx} className="bg-slate-800 p-6 md:p-8 rounded-2xl border border-slate-700 flex flex-col items-start hover:border-emerald-500/50 transition-colors">
                  <div className="text-2xl md:text-4xl font-black text-slate-700 mb-3 md:mb-4">0{idx + 1}</div>
                  <h3 className="text-lg md:text-2xl font-bold text-white mb-2 md:mb-4 break-keep">{card.title}</h3>
                  <p className="text-sm md:text-lg text-slate-400 leading-relaxed break-keep">{card.desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      case 'quote':
        return (
          <div className="flex flex-col items-center justify-center h-full text-center px-6 md:px-12">
            <h2 className="text-xl md:text-3xl font-bold text-emerald-400 mb-8 md:mb-16 break-keep">{slide.title}</h2>
            <blockquote className="text-3xl md:text-5xl lg:text-6xl font-black text-white leading-tight mb-8 relative break-keep">
              <span className="text-emerald-500/30 text-5xl md:text-8xl absolute -top-6 -left-6 md:-top-8 md:-left-12">"</span>
              {slide.quote}
              <span className="text-emerald-500/30 text-5xl md:text-8xl absolute -bottom-10 -right-4 md:-bottom-16 md:-right-8">"</span>
            </blockquote>
      ...
      default:
        return (
          <div className="flex flex-col h-full w-full justify-center px-4">
            ...
            <p className="text-base md:text-xl text-slate-300 leading-relaxed mb-8 md:mb-12 max-w-4xl text-center md:text-left break-keep">
              {slide.text}
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              {slide.points.map((point, idx) => (
                <div key={idx} className="flex items-center bg-slate-800/50 p-4 md:p-6 rounded-xl border border-slate-700">
                  <Play className="w-4 h-4 md:w-5 md:h-5 text-emerald-400 mr-3 md:mr-4 shrink-0" />
                  <span className="text-sm md:text-lg text-slate-200 font-bold break-keep">{point}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    // 💡 화면 전체를 덮는 Modal 배경 설정 (fixed, z-[100])
    <div className="fixed inset-0 z-[100] flex h-screen w-full bg-[#030712] text-slate-100 font-sans overflow-hidden">
      
      {/* 💡 스토리북 닫기 버튼 */}
      <button 
        onClick={onClose}
        className="absolute top-4 right-4 z-[110] bg-slate-800 hover:bg-red-500/20 hover:text-red-400 text-white p-3 rounded-full transition-all border border-slate-700"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
      </button>

      <button className="md:hidden absolute top-4 left-4 z-50 p-2 bg-slate-800 rounded-lg text-white" onClick={() => setIsMenuOpen(!isMenuOpen)}>
        <Menu className="w-6 h-6" />
      </button>

      <aside className={`absolute md:relative w-72 md:w-80 h-full bg-slate-900 border-r border-slate-800 flex flex-col z-40 transition-transform duration-300 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-6 md:p-8 border-b border-slate-800 mt-10 md:mt-0">
          <div className="flex items-center gap-3 mb-2">
            <Target className="w-6 h-6 md:w-8 md:h-8 text-emerald-500" />
            <h1 className="text-xl md:text-2xl font-bold text-white">FunnelBook</h1>
          </div>
          <p className="text-xs md:text-sm text-slate-500">The Creators AI 독점 가이드</p>
        </div>
          ...
          {CHAPTERS.map((chapter) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-4 px-4 py-3 md:py-4 rounded-xl transition-all duration-200 cursor-pointer ${
                  isActive ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200 border border-transparent'
                }`}
              >
                <IconComponent className={`w-4 h-4 md:w-5 md:h-5 ${isActive ? 'text-emerald-400' : 'text-slate-500'}`} />
        ...
        <div className="p-4 md:p-6 bg-slate-800/50 m-4 rounded-xl border border-slate-700">
          <p className="text-xs md:text-sm text-slate-400 mb-2 font-bold">진행률</p>
          <div className="w-full bg-slate-900 rounded-full h-1.5 md:h-2">
            <div className="bg-emerald-500 h-1.5 md:h-2 rounded-full transition-all duration-500" style={{ width: `${((currentSlide + 1) / totalSlides) * 100}%` }}></div>
          </div>
          <div className="mt-2 text-right text-[10px] md:text-xs font-bold text-slate-500">{currentSlide + 1} / {totalSlides}</div>
        </div>
      </aside>

      {isMenuOpen && <div className="absolute inset-0 bg-black/50 z-30 md:hidden" onClick={() => setIsMenuOpen(false)}></div>}

      <main className="flex-1 flex flex-col relative bg-[#0B1120] overflow-hidden">
        <div className="absolute top-0 right-0 w-[400px] md:w-[800px] h-[400px] md:h-[800px] bg-emerald-500/5 rounded-full blur-[80px] md:blur-[120px] -translate-y-1/2 translate-x-1/3 pointer-events-none"></div>
        <div className="flex-1 flex items-center justify-center p-4 md:p-16 relative z-10 overflow-y-auto pt-16 md:pt-16">
        ...
        <div className="relative h-20 md:h-24 border-t border-slate-800/50 bg-slate-900/80 backdrop-blur-md px-4 md:px-12 flex items-center justify-between z-20 shrink-0">
          <button type="button" onClick={() => handlePageChange(currentSlide - 1)} disabled={currentSlide === 0}
            ...
            <ChevronLeft className="w-4 h-4 md:w-5 md:h-5" /> <span className="hidden sm:inline">이전 페이지</span><span className="sm:hidden">이전</span>
          </button>

          <div className="flex gap-1.5 md:gap-2">
            {SLIDES.map((_, idx) => (
              <button type="button" key={idx} onClick={() => handlePageChange(idx)}
                className={`h-1.5 md:h-2 rounded-full transition-all duration-300 cursor-pointer ${currentSlide === idx ? 'w-6 md:w-8 bg-emerald-500' : 'w-1.5 md:w-2 bg-slate-700 hover:bg-slate-500'}`} />
            ))}
          </div>

          <button type="button" onClick={() => handlePageChange(currentSlide + 1)} disabled={currentSlide === totalSlides - 1}
            className={`flex items-center gap-1 md:gap-2 px-3 md:px-6 py-2 md:py-3 rounded-full text-sm md:text-base font-bold transition-colors cursor-pointer ${currentSlide === totalSlides - 1 ? 'text-slate-600 cursor-not-allowed' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500 hover:text-white'}`}>
            <span className="hidden sm:inline">다음 페이지</span><span className="sm:hidden">다음</span> <ChevronRight className="w-4 h-4 md:w-5 md:h-5" />
          </button>
        </div>
      </main>
    </div>
